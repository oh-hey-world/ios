...
-(id)initWithItems:(NSArray*)_items
  ...
  if (self) {
    ...
    [selectedModeButton setTitle:[NSString stringWithFormat:@"Selected (%u)", self.selectedItems.count] forState:UIControlStateNormal];
...
-(void)setCurrentSelectedItems {
  NSMutableArray *matchingItems = [[NSMutableArray alloc] initWithCapacity:items.count];
  for (KNSelectorItem *item in items) {
    if (item.selected == YES) {
      [matchingItems addObject:item];
    }
  }
  self.selectedItems = [NSArray arrayWithArray:matchingItems];
}
